/// Text names for the four modifier keys. keypeek uses Phosphor icon glyphs
/// here for the live overlay; a printed PDF has no icon font embedded, so
/// this copy is plain text throughout (Linux-only target — no macOS/Windows
/// glyph variants needed).
pub mod modifier_symbols {
    ...
    /// Build a standalone modifier key.
    ...
    /// Combined label for a set of held modifiers (e.g. "Ctrl+Shift"), with a short form to shrink.
